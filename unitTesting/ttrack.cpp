...
void increaseProblem( const double & DELTA,
		      ...
		      const std::vector<unsigned int>& NRA,
		      std::vector<Eigen::MatrixXd> & J,
		      std::vector<Eigen::MatrixXd> & Xhifree,
		      std::vector<Eigen::MatrixXd> & Jfree,
		      std::vector<Eigen::MatrixXd> & Xhilinked,
		      std::vector<Eigen::MatrixXd> & Alinked,
  ...
  for( unsigned int s=0;s<NB_STAGE;++s )
      ...
      if( RANKLINKED[s]>0 )
	{
	  ...
	  for( unsigned int sb=0;sb<s;++sb )
	    {
	      J[s] += Xhilinked[s]*Alinked[s].block(0,NRA[sb],RANKLINKED[s],NR[sb])*J[sb];
	    }
	}
...
{
  static int time; static bool touched;
...
# ifndef NDEBUG
  ...
  for( unsigned int s=0;s<NB_STAGE;++s )
    {
      /* NRA[s] is the size of all previous stages 0:s-1 stacked. */
      if( s==0 ) NRA[s]=0; else NRA[s]=NRA[s-1]+NR[s-1];
      b[s].resize(NR[s]);
      assert( (RANKFREE[s]>0)||(RANKLINKED[s]>0) );
  ...
  for( unsigned int i=0;i<NB_STAGE;++i )
    {
      ...
      sotDEBUG(1) << "J"<<i+1<<" = " << (soth::MATLAB)hcod[i].getJ() << std::endl;
  ...
  for( unsigned int t=0;t<100000;t++ )
    {
      sotDEBUG(1) << " --- TIME t=" << t << " -----------------------------" << std::endl;
      coutListen.inc();
      increaseProblem(1e-3,
		      NB_STAGE, NC, NR, RANKLINKED, RANKFREE, NRA,
		      J, Xhifree, Jfree, Xhilinked, Alinked, b );
      for( unsigned int i=0;i<NB_STAGE;++i )
	hcod[i].getBoundsInternal() = b[i];
      hcod.activeSearch(solution);
    }
