  ...
  for( unsigned int k=0;k<NB_STAGE;++k )
	...
	Z = Aksvd.matrixV().rightCols(rank);
      */
      ...
      for(int i=0;i<ratioSvd;++i)
	Akcod.compute(A[k]*Z.rightCols(rank));
...
void hcod( std::vector<Eigen::MatrixXd> &A,
	   std::vector<soth::VectorXd> &b,
	   const unsigned int NB_STAGE,
	   const std::vector<unsigned int> & RANKFREE,
	   const std::vector<unsigned int> & /*RANKLINKED*/,
...
# ifndef NDEBUG
  ...
  boost::program_options::variables_map optionMap;
  {
    ...
    desc.add_options()
      ...
      ("denoise,n",po::value<int>(),"Run n times the algo to remove the computation-time noise")
      ("ratio,r",po::value<int>(),"ratio of the SVD wrt COD")
      ("phase,p",po::value<int>(),"Phase of the algo 1 = Decompo, 2 = inversion, 3 = projection")
      ;
...
# ifndef NDEBUG
  ...
  iss <<"/tmp/ttro_"
      ...
      <<optionMap["method"].as<int>()<<"_"
      <<optionMap["phase"].as<int>()<<"_"
      <<optionMap["ratio"].as<int>()<<".dat";
  ...
  for( int shoot=0;shoot<1000;shoot++ )
    {
      ...
      generateFixedSizeRandomProfile(optionMap["size"].as<int>(),
				     1.2,0.8,0.5,NB_STAGE,RANKFREE,RANKLINKED,NR,NC);
      generateDeficientDataSet(J,b,NB_STAGE,RANKFREE,RANKLINKED,NR,NC);
      e.resize(NB_STAGE);
      for( unsigned int i=0;i<NB_STAGE;++i )
	{
